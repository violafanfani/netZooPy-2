...
class AggroLioness(Lioness):
    """ Using LIONESS to infer group gene regulatory networks.
        1. Reading in PANDA network and preprocessed middle data
        2. Computing coexpression network
        3. Normalizing coexpression network
        4. Running PANDA algorithm
        5. Writing out AggroPanda networks
    ...
    ----------
            ...
            groups          : dict
                Dictionary with the aggregate group sample names,  group:[sample_a, sample_b...] 
            groups_index     : dict
                Dictionary with the aggregate group sample indices,  group:[index_a, index_b...]. 
                This is None by default, as it is preferrable to pass only the sample names via groups    
            computing       : str
    ...
    def __init__(
        self,
        obj,
        groups,#groups to compute the aggregate networks
        groups_index = None, #One could pass index groups, but this is not recommended
        computing="cpu",
        precision="double",
        ncores=1,
        start=1,
        end=None,
        subset_numbers=None,#TODO:remove
        subset_names=None,#TODO:remove
        ...
        ### Now we are passing a dictionary with the groups to be computed, 
        ### Hence one could just pass the dictionary of the groups of interest.
        ...
        #     # which samples need to be analysed. The background is always what is used for PANDA
        #     # and stays the same
        ...
        ### #TODO: fix here
        #self.n_lio_samples = len(self.groups.keys())
        if self.groups_index==None:
            # We create the index dictionary that we use to run lioness
            self.groups_index = {}
            self.all_samples_in_groups = set()
            for k,v in self.groups.items():
                # First we check there are no overlaps
                temp = self.all_samples_in_groups.union(set(v))
                if len(temp)<(len(self.all_samples_in_groups)+len(v)):
                    print('WARNING: group %s has overlapping samples with other groups')
                self.all_samples_in_groups = temp.copy()
                # Now we compute the indices of interest
                self.groups_index[k] = [list(self.expression_samples).index(i) for i in v]
        else:
            self.all_samples_in_groups = set()
            for k,v in self.groups_index.items():
                # First we check there are no overlaps
                temp = self.all_samples_in_groups.union(set(v))
                if len(temp)<(len(self.all_samples_in_groups)+len(v)):
                    print('WARNING: group %s has overlapping samples with other groups')
        ...
        self.n_groups = len(self.groups_index.keys())
        self.group_names = list(self.groups_index.keys())
        print("Number of total samples:", self.n_conditions)
        print("Number of groups:", self.n_groups)
        print("Number of computed samples:", len(self.all_samples_in_groups))
        ...
        # Run LIONESS
        #############
        if int(self.n_groups) >= int(self.n_cores) and self.computing == "cpu":
            ...
            self.total_lioness_network = Parallel(n_jobs=self.n_cores)(
                self.__par_lioness_loop(i, output) for i in self.group_names
            ) 
        ...
        if self.ignore_final:
            ...
            if output == "network":
                if isinstance(origmotif, pd.DataFrame):
                    ...
                    indDF = indDF.append(
                        pd.DataFrame(self.total_lioness_network, index = self.group_names)
                    ).transpose()
                ...
                self.export_lioness_results = pd.DataFrame(
                    self.total_lioness_network, columns=gene_names, index =self.group_name
                ).transpose()
            elif output == "tf_targeting":
                self.export_lioness_results = pd.DataFrame(
                    self.total_lioness_network, columns=tf_names, index = self.group_name
                ).transpose()
    ...
    def __par_lioness_loop(self, group_name, output):
        """ Running one iteration of lioness on the input group 
        Returns
        ---------
            self.total_lioness_network: array
                An edge-by-sample matrix containing sample-specific networks.
        ...
        # For consistency with R, we are using the N panda_all - (N-1) panda_all_but_q
        #lioness_network = self.n_conditions * (self.network - subset_panda_network) + subset_panda_network
        # Here we use (N-k) as weight, where k is the number of samples in the group
        lioness_network = (self.n_conditions * self.network) - (
            (self.n_conditions - k) * subset_panda_network
        )
        ...
        if self.save_single:
            ...
            with Timer(
                ...
                path = os.path.join(self.save_dir, "aggrolioness.%s.%s" % (group_name,self.save_fmt))
                print(path)
                self.__lioness_to_disk(lioness_network, path)
